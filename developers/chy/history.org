* [2.6.22         ] i386-pit-stop-only-when-in-periodic-or-oneshot-mode.patch {C::fixbug::hardware::time::hardware::add an "if" statement to config mode PERIODIC or ONESHOT specificly}
  + [[file:2.6.22/i386-pit-stop-only-when-in-periodic-or-oneshot-mode.patch][2.6.22]]  {MOD::arch/i386}
  - 2.6.23  {M}
* [2.6.22         ] clockevents-fix-device-replacement.patch {C::fixbug::hardware::hardware::hardware::add an "else" statement to clear the tick_broadcast_mask when a higher rated device is used}
...
* [2.6.22         ] hrtimer-no-getnstimeofday.patch {C::performance::hrtimer::replace getnstimeofday with other functions}
  ...
  - 2.6.23
* [2.6.22         ] nohz-fix-nohz-x86-dyntick-idle-handling.patch {C::fixbug::crash::hardware::hardware::add cond-compile for some arch where tick nohz handling went wrong}
  + [[file:2.6.22/nohz-fix-nohz-x86-dyntick-idle-handling.patch][2.6.22]]   {MOD::arch/i386}
  - 2.6.23
* [2.6.22         ] clockevents-fix-resume-logic.patch {C::fixbug::idle::order::sync::resume clockevent device before resume tick}
...
* [2.6.22         ] highres-improve-debug-output.patch {C::feature::debuginfo::debuginfo for hrtimer and clock events}
  ...
  - 2.6.23
* [2.6.22         ] cpuidle_hang_fix.patch {C::fixbug::hang::order::sync::idle handler to enable intr before returning from idle handler, set current driver to NULL when fail to attach on all devices}
...
* [2.6.22         ] hrtimer-speedup-hrtimer_enqueue.patch {C::performance::hrtimer::speedup hrtimer_enqueue}
  ...
  - 2.6.23
* [2.6.22         ] acpi-move-timer-broadcast-and-pmtimer-access-before-c3-arbiter-shutdown.patch {C::fixbug::hardware::irq::hardware::check ARB_DIS status before reading from ICH deivce}
...
* [2.6.22 - 2.6.26] nf_conntrack-fix-smp-processor-id.patch {C::fixbug::data_err::preempt::preempt::replace with raw}
  ...
  - 2.6.29
* [2.6.22 - 2.6.26] preempt-realtime-powerpc-b3.patch {C::fixbug::crash::migration::migration::get the right cpu_mask}
